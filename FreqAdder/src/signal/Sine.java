...
/**
 * Class to create sinusoidal audio signal. It extends from Tone class 
 * @author Ehsan-Ul-Haq
 * @Date 3 February 2016
 * @version 1.0
 */
public class Sine extends Tone{
	 ...
	 */
	public Sine(FreqObject dao, int waveSelection) {
	...
	public void generateSignal(int sample, int waveSelection) {
		...
		if(waveSelection==0)
		{
			buffer[sample] = (float) ((float)amp_1 * Math.sin(twoPiF1*time + Math.toRadians( phase_1 )));
		}
		
		else if(waveSelection==1)
		{
			buffer[sample] = (float) ((float)amp_2 * Math.sin(twoPiF2*time + Math.toRadians( phase_2 )));
		}
		else if(waveSelection==2)
		{
			double f1Component =  0.5 * amp_1 * Math.sin(twoPiF1 * time + Math.toRadians( phase_1 ));
			double f2Component =  0.5 * amp_2 * Math.sin(twoPiF2 * time + Math.toRadians( phase_2 ));
